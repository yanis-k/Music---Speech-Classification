...
alldata_raw = rbind(temp, read.csv("F:/Yanis/MEGA/UniProjects/AVT/Datasets/Final/LV/dataset_ETI.csv"))
...
for (gamma in gammavalues) {
  ...
  for(i in 1:k){
    ...
    pred = predict(svm_model, testset[,-66])
    ...
    testsets <- rbind(testsets, as.data.frame(testset[, 66]))
  }
...
for(i in 1:k){
  ...
  pred = predict(svm_model, testset[,-66], type="raw")
