...
class_train = alldata_unique_train[, 14]    #split training data columns into features(cols 1-13) and class(col 14)
data_feats_train = alldata_unique_train[, -14]

class_test = alldata_unique_test[, 14]    #split testing data columns into features(cols 1-13) and class(col 14)
...
for (gamma in gammavalues) {
  ...
  pred = predict(svm_model, data_test[,-14])
  ...
  testsets <- rbind(testsets, as.data.frame(data_test[, 14]))
...
pred = predict(svm_model, data_test[,-14], type="raw")
