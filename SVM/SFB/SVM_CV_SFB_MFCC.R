...
class = alldata_unique[, 14]    #split data columns into features(cols 1-13) and class(col 14)
...
data_feats_pc <- as.data.frame(predict(pca_model, data_feats)[, 1:13]) #Keeping feats 1-13
...
for (gamma in gammavalues) {
  ...
  for(i in 1:k){
    ...
    pred = predict(svm_model, testset[,-14])
    ...
    testsets <- rbind(testsets, as.data.frame(testset[, 14]))
  }
...
for(i in 1:k){
  ...
  pred = predict(svm_model, testset[,-14], type="raw")
