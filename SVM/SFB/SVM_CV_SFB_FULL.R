...
alldata_raw = rbind(temp, read.csv("F:/Yanis/MEGA/UniProjects/AVT/Datasets/Final/LV/dataset_SFB.csv"))

#Preprocessing
alldata_raw = unique(alldata_raw) #delete duplicate entries
alldata_unique  <- alldata_raw[complete.cases(alldata_raw), ]

class = alldata_unique[, 22]    #split data columns into features(cols 1-21) and class(col 22)
...
data_feats_pc <- as.data.frame(predict(pca_model, data_feats)[, 1:16]) #Keeping feats 1-16
data_feats_pc[, 17:21] <- 0 #Ignore feats 17-21
...
data_feats_rec[, 17:21] <- NULL #Throw Away Components 17-21
...
for (gamma in gammavalues) {
  ...
  for(i in 1:k){
    ...
    pred = predict(svm_model, testset[,-17])
    ...
    testsets <- rbind(testsets, as.data.frame(testset[, 17]))
  }
...
for(i in 1:k){
  ...
  pred = predict(svm_model, testset[,-17], type="raw")
