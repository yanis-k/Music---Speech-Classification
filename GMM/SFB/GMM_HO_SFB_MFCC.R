...
# Load the data
alldata_raw_train = read.csv("dataset_SFB.csv")  #Windows Dir
alldata_raw_test = read.csv("dataset_SFB.csv")  #Windows Dir

alldata_raw_train$rms <- NULL
alldata_raw_test$rms <- NULL

alldata_raw_train$zerocross <- NULL
alldata_raw_test$zerocross <- NULL

alldata_raw_train$rolloff <- NULL
alldata_raw_test$rolloff <- NULL

alldata_raw_train$centroid <- NULL
alldata_raw_test$centroid <- NULL

alldata_raw_train$kurtosis <- NULL
alldata_raw_test$kurtosis <- NULL

alldata_raw_train$spread <- NULL
alldata_raw_test$spread <- NULL

alldata_raw_train$flatness <- NULL
alldata_raw_test$flatness <- NULL

alldata_raw_train$skewness <- NULL
alldata_raw_test$skewness <- NULL
...
class_train = alldata_unique_train[, 14]    #split training data columns into features(cols 1-20) and class(col 21)
data_feats_train = alldata_unique_train[, -14]

class_test = alldata_unique_test[, 14]    #split testing data columns into features(cols 1-20) and class(col 21)
data_feats_test = alldata_unique_test[, -14]
...
data_feats_pc_train <- as.data.frame(predict(pca_model_train, data_feats_train)[, 1:13]) #Keeping feats 1-15
...
data_feats_pc_test <- as.data.frame(predict(pca_model_test, data_feats_test)[, 1:13]) #Keeping feats 1-15
...
gmm <- MclustDA(data_train[,-14], data_train[,14])

summary.MclustDA(gmm, parameters = FALSE)
summary.MclustDA(gmm, parameters = FALSE, data_test[,-14], data_test[, 14])

pred_train <- predict.MclustDA(gmm)
pred_test <- predict.MclustDA(gmm, data_test[, -14])
opt_acc = Accuracy(pred_test$classification, data_test[, 14])
opt_prec = Precision(data_test[, 14], pred_test$classification)
opt_rec = Recall(data_test[, 14], pred_test$classification)
...
#Designing ROC Curve
roc.curve(data_test[, 14], pred_test$classification)
