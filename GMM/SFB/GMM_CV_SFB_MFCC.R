...
alldata_raw$zerocross <- NULL
alldata_raw$rolloff <- NULL
alldata_raw$centroid <- NULL
alldata_raw$kurtosis <- NULL
alldata_raw$spread <- NULL
alldata_raw$flatness <- NULL
alldata_raw$skewness <- NULL

#Preprocessing
alldata_raw = unique(alldata_raw) #delete duplicate entries
alldata_unique  <- alldata_raw[complete.cases(alldata_raw), ]

class = alldata_unique[, 14]    #split data columns into features(cols 1-16) and class(col 17)
data_feats = alldata_unique[, -14]
cor(data_feats)
...
data_feats_pc <- as.data.frame(predict(pca_model, data_feats)[, 1:13]) #Keeping feats 1-12
...
gmm <- MclustDA(data[,-14], data[,14])
pred <- cvMclustDA(gmm, nfold=6)
str(pred)

opt_accuracy = Accuracy(pred$classification, data[, 14]) #Calculating Optimal Accuracy
opt_precision = Precision(data[, 14], pred$classification, "music") #Calculating Optimal Precision
opt_recall = Recall(data[, 14], pred$classification, "music") #Calculating Optimal Recall
...
roc.curve(pred$classification, data[, 14])