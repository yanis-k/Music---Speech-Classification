...
class_train = alldata_unique_train[, 43]    #split training data columns into features(cols 1-21) and class(col 22)
data_feats_train = alldata_unique_train[, -43]

class_test = alldata_unique_test[, 43]    #split testing data columns into features(cols 1-21) and class(col 22)
data_feats_test = alldata_unique_test[, -43]
...
data_feats_pc_train <- as.data.frame(predict(pca_model_train, data_feats_train)[, 1:32]) #Keeping feats 1-16
data_feats_pc_train[, 33:42] <- 0 #Ignore feats 17-21
...
data_feats_rec_train[, 33:42] <- NULL #Throw Away Components 17-21
...
data_feats_pc_test <- as.data.frame(predict(pca_model_test, data_feats_test)[, 1:32]) #Keeping feats 1-16
data_feats_pc_test[, 33:42] <- 0 #Ignore feats 17-21
...
data_feats_rec_test[, 33:42] <- NULL #Throw Away Components 17-21
data_test = cbind(data_feats_rec_test, class_test) #compiling final testing dataset

gmm <- MclustDA(data_train[,-33], data_train[,33])

summary.MclustDA(gmm, parameters = FALSE)
summary.MclustDA(gmm, parameters = FALSE, data_test[,-33], data_test[, 33])

pred_train <- predict.MclustDA(gmm)
pred_test <- predict.MclustDA(gmm, data_test[, -33])
opt_acc = Accuracy(pred_test$classification, data_test[,33])
opt_prec = Precision(data_test[, 33], pred_test$classification)
opt_rec = Recall(data_test[, 33], pred_test$classification)
...
#Designing ROC Curve
roc.curve(data_test[, 33], pred_test$classification)