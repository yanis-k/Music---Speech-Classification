...
class = alldata_unique[, 41]    #split data columns into features(cols 1-16) and class(col 17)
data_feats = alldata_unique[, -41]
cor(data_feats)
...
data_feats_pc <- as.data.frame(predict(pca_model, data_feats)[, 1:30]) #Keeping feats 1-12
data_feats_pc[, 31:40] <- 0 #Ignore feats 13-16
...
data_feats_rec[, 31:40] <- NULL #Throw Away Components 14-16
data = cbind(data_feats_rec, class) #compiling final dataset

gmm <- MclustDA(data[,-31], data[,31])
pred <- cvMclustDA(gmm, nfold=6)
str(pred)

opt_accuracy = Accuracy(pred$classification, data[, 31]) #Calculating Optimal Accuracy
opt_precision = Precision(data[, 31], pred$classification, "music") #Calculating Optimal Precision
opt_recall = Recall(data[, 31], pred$classification, "music") #Calculating Optimal Recall
...
roc.curve(pred$classification, data[, 31])