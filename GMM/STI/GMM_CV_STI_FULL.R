...
# Load the data
alldata_raw = read.csv("home/yanis/MEGA/UniProjects/AVT/Datasets/Final/Mirex/dataset_STI.csv")  #Windows Dir
temp <- c()
music <- c()
speech <- c()
...
alldata_raw = rbind(temp, read.csv("/home/yanis/MEGA/UniProjects/AVT/Datasets/Final/LV/dataset_STI.csv"))

#Preprocessing
alldata_raw = unique(alldata_raw) #delete duplicate entries
alldata_unique  <- alldata_raw[complete.cases(alldata_raw), ]

class = alldata_unique[, 43]    #split data columns into features(cols 1-16) and class(col 17)
data_feats = alldata_unique[, -43]
cor(data_feats)
...
data_feats_pc <- as.data.frame(predict(pca_model, data_feats)[, 1:32]) #Keeping feats 1-12
data_feats_pc[, 33:42] <- 0 #Ignore feats 13-16
...
data_feats_rec[, 33:42] <- NULL #Throw Away Components 14-16
data = cbind(data_feats_rec, class) #compiling final dataset


gmm <- MclustDA(data[,-33], data[,33])
pred <- cvMclustDA(gmm, nfold=6)
str(pred)

opt_accuracy = Accuracy(pred$classification, data[, 33]) #Calculating Optimal Accuracy
opt_precision = Precision(data[, 33], pred$classification, "music") #Calculating Optimal Precision
opt_recall = Recall(data[, 33], pred$classification, "music") #Calculating Optimal Recall
...
roc.curve(pred$classification, data[, 33])