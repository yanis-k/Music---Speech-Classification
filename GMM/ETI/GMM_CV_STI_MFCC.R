...
alldata_raw$rms_mcr <- NULL
alldata_raw$zerocross_mcr <- NULL
alldata_raw$roll.off_mcr <- NULL
alldata_raw$centroid_mcr <- NULL
alldata_raw$kurtosis_mcr <- NULL
alldata_raw$spread_mcr <- NULL
alldata_raw$fltaness_mcr <- NULL
alldata_raw$skewness_mcr <- NULL

alldata_raw$rms_masd <- NULL
alldata_raw$zerocross_masd <- NULL
alldata_raw$roll.off_masd <- NULL
alldata_raw$centroid_masd <- NULL
alldata_raw$kurtosis_masd <- NULL
alldata_raw$spread_masd <- NULL
alldata_raw$fltaness_masd <- NULL
alldata_raw$skewness_masd <- NULL

alldata_raw$rms_flat <- NULL
alldata_raw$zerocross_flat <- NULL
alldata_raw$roll.off_flat <- NULL
alldata_raw$centroid_flat <- NULL
alldata_raw$kurtosis_flat <- NULL
alldata_raw$spread_flat <- NULL
alldata_raw$fltaness_flat <- NULL
alldata_raw$skewness_flat <- NULL

#Preprocessing
alldata_raw = unique(alldata_raw) #delete duplicate entries
alldata_unique  <- alldata_raw[complete.cases(alldata_raw), ]

class = alldata_unique[, 66]    #split data columns into features(cols 1-16) and class(col 17)
data_feats = alldata_unique[, -66]
cor(data_feats)
...
data_feats_pc <- as.data.frame(predict(pca_model, data_feats)[, 1:65]) #Keeping feats 1-12
...
gmm <- MclustDA(data[, -66], data[, 66])
pred <- cvMclustDA(gmm, nfold=6)
str(pred)

opt_accuracy = Accuracy(pred$classification, data[, 66]) #Calculating Optimal Accuracy
opt_precision = Precision(data[, 66], pred$classification, "music") #Calculating Optimal Precision
opt_recall = Recall(data[, 66], pred$classification, "music") #Calculating Optimal Recall
...
roc.curve(pred$classification, data[, 66])