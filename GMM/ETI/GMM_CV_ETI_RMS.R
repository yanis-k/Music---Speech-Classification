...
class = alldata_unique[, 101]    #split data columns into features(cols 1-16) and class(col 17)
data_feats = alldata_unique[, -101]
cor(data_feats)
...
data_feats_pc <- as.data.frame(predict(pca_model, data_feats)[, 1:75]) #Keeping feats 1-12
data_feats_pc[, 76:100] <- 0 #Ignore feats 13-16
...
data_feats_rec[, 76:100] <- NULL #Throw Away Components 14-16
data = cbind(data_feats_rec, class) #compiling final dataset

gmm <- MclustDA(data[, -76], data[, 76])
pred <- cvMclustDA(gmm, nfold=6)
str(pred)

opt_accuracy = Accuracy(pred$classification, data[, 76]) #Calculating Optimal Accuracy
opt_precision = Precision(data[, 76], pred$classification, "music") #Calculating Optimal Precision
opt_recall = Recall(data[, 76], pred$classification, "music") #Calculating Optimal Recall
...
roc.curve(pred$classification, data[, 76])